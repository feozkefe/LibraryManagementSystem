...
namespace Library.WinUI
{
    public partial class BookIssuePage : Form
    {
        ...
        private void txtstdsearch_TextChanged(object sender, EventArgs e)
        {
            datastudent.DataSource = service.StudentService.GetByCondition(x => x.StudentName.ToLower().Contains(txtstdsearch.Text)).ToList();
        }

        private void txtbooksearch_TextChanged(object sender, EventArgs e)
        {
            databook.DataSource = service.BooksService.GetByCondition(x => x.BookName.ToLower().Contains(txtbooksearch.Text)).ToList();
        }
    }
}
